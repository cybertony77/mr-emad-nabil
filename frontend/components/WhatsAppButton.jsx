  ...
  const handleWhatsAppClick = () => {
    ...
    try {
      ...
      const currentWeek = {
        ...
        lastAttendanceCenter: student.lastAttendanceCenter || (weekData ? weekData.lastAttendanceCenter : null) || null,
      ...
      let attendanceInfo = 'غائب';
      if (currentWeek.attended && currentWeek.lastAttendance && currentWeek.lastAttendance !== 'N/A') {
        // Extract date from lastAttendance (format: "04/02/2026 at 04:52 PM")
        const dateMatch = currentWeek.lastAttendance.match(/^(\d{2}\/\d{2}\/\d{4})/);
        const extractedDate = dateMatch ? dateMatch[1] : currentWeek.lastAttendance.split(' at ')[0];
        const center = currentWeek.lastAttendanceCenter || '';
        attendanceInfo = `${extractedDate} في ${center}`;
      }
      
      // Format homework status
      let homeworkStatus = '';
      if (currentWeek.hwDone === true) {
        homeworkStatus = 'تم';
      } else if (currentWeek.hwDone === false) {
        homeworkStatus = 'لم يتم';
      } else if (currentWeek.hwDone === 'Not Completed') {
        homeworkStatus = 'لم يكتمل';
      } else if (currentWeek.hwDone === 'No Homework') {
        homeworkStatus = 'لا واجبات منزلية';
      } else {
        homeworkStatus = 'لم يتم'; // Default fallback
      }
      
      // Format quiz degree
      let quizDegreeText = '';
      if (currentWeek.quizDegree !== null && String(currentWeek.quizDegree).trim() !== '') {
        const quizDegreeStr = String(currentWeek.quizDegree).trim();
        if (quizDegreeStr === 'Didn\'t Attend The Quiz') {
          quizDegreeText = 'لم يحضر الاختبار';
        } else if (quizDegreeStr === 'No Quiz') {
          quizDegreeText = 'لا يوجد اختبار';
        } else {
          quizDegreeText = quizDegreeStr;
        }
      }
      ...
      // Create the Arabic message
      let whatsappMessage = `عزيزي ولي أمر ${firstName}،
نود إعلامكم أننا في:
...
🖇️ متابعة الحضور: ${publicLink}

دائمًا يسعدنا البقاء على تواصل 😊❤
